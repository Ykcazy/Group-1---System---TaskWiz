defmodule TaskManager.Cli do
  ...
  defp format_task_without_id(%TaskManager.Tasks.Task{id: _id, title: title, status: status,
  ...
  def view_all_tasks() do
    if Enum.empty?(Tasks.list_tasks()) do
      ...
      # Fetch the tasks and exclude the ID field
      tasks = Tasks.list_tasks()
      formatted_tasks = Enum.map(tasks, &display_task/1)

      # Use table_rex to generate a table
      table = TableRex.print(formatted_tasks, columns: ~w(Title Status Description Due_Date))
