defmodule TaskManager.Cli do
  ...
  defp display_task(%TaskManager.Tasks.Task{id: _id, title: title, status: status, description: description, due_date: due_date, inserted_at: _inserted_at, updated_at: _updated_at}) do
    ...
    # Format each field with right alignment using String.pad_leading/2
    formatted_title = String.pad_leading(title, title_width, " ")
    formatted_status = String.pad_leading(status, status_width, " ")
    formatted_description = String.pad_leading(description, description_width, " ")
    formatted_due_date = String.pad_leading(due_date, due_date_width, " ")

    # Display the header row
    if id == :id do
      IO.puts("| ID | Title | Status | Description | Due Date |")
      IO.puts("|----|-------|--------|-------------|----------|")
    end

    # Display the formatted table row
    IO.puts("| #{formatted_title} | #{formatted_status} | #{formatted_description} | #{formatted_due_date} |")
  end
