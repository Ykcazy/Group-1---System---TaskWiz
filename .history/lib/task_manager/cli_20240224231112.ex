defmodule TaskManager.Cli do
  ...
  defp display_task(%TaskManager.Tasks.Task{id: _id, title: title, status: status, description: description, due_date: due_date, inserted_at: _inserted_at, updated_at: _updated_at}) do
    ...
    # Format each field with right alignment
    formatted_title = String.rjust(title, title_width)
    formatted_status = String.rjust(status, status_width)
    formatted_description = String.rjust(description, description_width)
    formatted_due_date = String.rjust(due_date, due_date_width)
