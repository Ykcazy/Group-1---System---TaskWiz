defmodule TaskManager.Cli do
  ...
  defp display_task(%TaskManager.Tasks.Task{id: _id, title: title, status: status,
    ...
    format = "~20s~15s~30s~12s"

    # Display the formatted table row
    IO.format(format, [title, status, description, due_date])
  end
  ...
  def view_all_tasks() do
    if Enum.empty?(Tasks.list_tasks()) do
      ...
      IO.format("~20s~15s~30s~12s", ["Title", "Status", "Description", "Due Date"])
      IO.puts("\n" <> String.duplicate("-", 20) <> String.duplicate("-", 15) <> String.duplicate("-", 30) <> String.duplicate("-", 12))
      Tasks.list_tasks()
